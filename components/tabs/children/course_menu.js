...
import { View  , Text , TouchableHighlight , StyleSheet , Image , ScrollView, Platform , ActivityIndicator , WebView , AsyncStorage } from 'react-native'
...
debugger;
    ...
    courseDefinition () 
    {
        ...
        AsyncStorage.getItem(title).then((success) => 
        {
            if (success == null) 
            {   
                let progressObj = {
            ...
            {
                console.log(`${title} has already been opened and saved`);
                console.log(success);

                var promise = JSON.parse(success);
                let progress = (promise.complete / promise.total).toFixed(2) * 100;
                ...
                <ScrollView>
                    <View style={[styles.buttonContainer,{justifyContent:'center',alignSelf:'center'}]}>  
                        ...
                        <View style={[styles.buttonContainer,{paddingTop:0,paddingBottom:0}]}>
                            <TouchableHighlight style={styles.button} onPress={()=>{ this.navigate('Quiz',{params:this.state.quiz,icon:this.state.icon,course_id:this.state.course_id,course_title : this.state.title}) }}>
                        ...
                        <View style={[styles.buttonContainer,{paddingTop:0,paddingBottom:0}]}>
                            <TouchableHighlight style={styles.button} onPress={()=>{ this.navigate('Quiz',{params:this.state.quiz,icon:this.state.icon,course_id:this.state.course_id,course_title : this.state.title}) }}>
                                <Text style={styles.textStyle}>Course Progress ({this.state.progress}) %  </Text>
                            </TouchableHighlight>
                            <Icon name="star" size={20} color="#00adee"/>
                        </View>
